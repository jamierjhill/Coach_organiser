...
I hope this email finds you well. Please find your invoice from your recent session below:
...
If you have any questions about this invoice or need to discuss payment arrangements, please contact me.
...
# Add this route to your existing blueprints/invoice.py file

@invoice_bp.route("/invoices/analytics")
@login_required
def invoice_analytics():
    """Analytics dashboard for coaching revenue and client insights."""
    try:
        invoices = load_json_feature(INVOICES_DIR, current_user.username)
        invoices = [normalize_invoice_data(inv) for inv in invoices]
        
        # Filter to paid invoices for revenue calculations
        paid_invoices = [inv for inv in invoices if inv.get("status") == "paid"]
        ...
        for invoice in paid_invoices:
            try:
                ...
                paid_date = invoice.get("paid_date") or invoice.get("issue_date", "")
                
                if not paid_date:
                    continue
                    
                # Parse date
                invoice_date = datetime.strptime(paid_date, "%Y-%m-%d")
                month_key = invoice_date.strftime("%Y-%m")
                quarter_key = f"{invoice_date.year}-Q{(invoice_date.month-1)//3 + 1}"
                year_key = str(invoice_date.year)
                
                # Aggregate by time periods
                revenue_by_month[month_key] = revenue_by_month.get(month_key, 0) + amount
                revenue_by_quarter[quarter_key] = revenue_by_quarter.get(quarter_key, 0) + amount
                revenue_by_year[year_key] = revenue_by_year.get(year_key, 0) + amount
                
                # Track client metrics
                client_revenue[client_name] = client_revenue.get(client_name, 0) + amount
                client_invoice_count[client_name] = client_invoice_count.get(client_name, 0) + 1
        ...
        months_data = []
        for month in sorted(revenue_by_month.keys(), reverse=True)[:12]:  # Last 12 months
            try:
        ...
        quarters_data = []
        for quarter in sorted(revenue_by_quarter.keys(), reverse=True)[:8]:  # Last 8 quarters
            quarters_data.append({
                'period': quarter,
                'revenue': revenue_by_quarter[quarter],
                'key': quarter
            })
        
        years_data = []
        for year in sorted(revenue_by_year.keys(), reverse=True):
            years_data.append({
                'period': year,
        ...
        top_clients = []
        for client_name in client_revenue:
            total_revenue = client_revenue[client_name]
            invoice_count = client_invoice_count[client_name]
            avg_invoice = total_revenue / invoice_count if invoice_count > 0 else 0
            
            # Get client email if available
            client_email = ""
            for invoice in invoices:
                if invoice.get("client_name") == client_name and invoice.get("client_email"):
                    client_email = invoice.get("client_email")
                    break
        ...
        # Current month/quarter/year revenue
        now = datetime.now()
        current_month = now.strftime("%Y-%m")
        current_quarter = f"{now.year}-Q{(now.month-1)//3 + 1}"
        current_year = str(now.year)
        
        current_month_revenue = revenue_by_month.get(current_month, 0)
        current_quarter_revenue = revenue_by_quarter.get(current_quarter, 0)
        current_year_revenue = revenue_by_year.get(current_year, 0)
        
        # Growth calculations (comparing to previous periods)
        prev_month = (now.replace(day=1) - timedelta(days=1)).strftime("%Y-%m")
        prev_month_revenue = revenue_by_month.get(prev_month, 0)
        month_growth = ((current_month_revenue - prev_month_revenue) / prev_month_revenue * 100) if prev_month_revenue > 0 else 0
        
        return render_template(
            "invoice_analytics.html",
            months_data=months_data[:6],  # Show last 6 months
            quarters_data=quarters_data[:4],  # Show last 4 quarters
            years_data=years_data,
            top_clients=top_clients[:10],  # Top 10 clients
            total_revenue=total_revenue,
            total_clients=total_clients,
            ...
            all_invoices_count=len(invoices)
        )
